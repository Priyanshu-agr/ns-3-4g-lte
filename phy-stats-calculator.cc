...
std::map<uint64_t, double> imsiSinrMap; // global or class member variable
                                              ...
                                              uint8_t componentCarrierId)
{
    ...
    imsiSinrMap[imsi] = sinr;
}

double
PhyStatsCalculator::GetSinrForImsi(uint64_t imsi)
{
    if (imsiSinrMap.find(imsi) != imsiSinrMap.end())
    {
        return imsiSinrMap[imsi];
    }
    else
    {
        return 0;
