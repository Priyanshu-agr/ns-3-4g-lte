...
std::map<uint64_t, uint32_t> imsiMcsMap; // global or class member variable
...
PhyRxStatsCalculator::DlPhyReception(PhyReceptionStatParameters params)
{
    ...
    // imsiMcsMap[params.m_imsi] = (uint32_t)params.m_mcs;
    imsiMcsMap[(uint64_t)params.m_imsi] = (uint32_t)params.m_mcs;
}

uint32_t
PhyRxStatsCalculator::GetMcsForImsi(uint64_t imsi)
{
    if (imsiMcsMap.find(imsi) != imsiMcsMap.end())
    {
        return imsiMcsMap[imsi];
    }
    else
    {
        return 0;
